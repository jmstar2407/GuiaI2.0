class Simulation {
    ...
    update() {
        ...
        // this.ctx.fillStyle = `rgba(6, 28, 43, 1)`;
...
let mouseBody = new Body(new Vector(0, 0), new Vector(0, 0), -1000, 0, true);
let bodyIndex = null;
...
window.wallpaperPropertyListener = {
    applyUserProperties: function(properties) {
        if (properties.particle_count) {
            if (properties.particle_count.value !== "") {
                redraw(properties.particle_count.value);
                particleCount = properties.particle_count.value;
